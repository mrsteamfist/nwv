...
namespace HadriansWall
{
    ...
    background:black;
        ...
        {
			var width = Math.Max(WebControl.Width, WebControl.Height);
			var height = Math.Min(WebControl.Width, WebControl.Height);
        ...
        ImageElement GenerateItem(string img, int center, int top, int width, int height)
        {
            var image = new ImageElement(img);
            image.Width = width;
            image.Height = height;
